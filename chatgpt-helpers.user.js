...
(function() {
    ...
    const getAllAnswers = (wip = false) => document.querySelectorAll(".markdown.prose.w-full.break-words.light" + (wip ? ".result-streaming" : ":not(.result-streaming)"))
    ...
    const mainLoop = setInterval(() => {
        ...
        if (!wipAnswers.length) //don't overwrite this unless a new one is found
        ...
        if (!initialRunDone) {
            if (document.getElementById("headlessui-portal-root")?.textContent.toLowerCase().includes("chatgpt plus is available"))
            ...
            //to get the anwer's content, innerText is better because it contains newlines: \n
